...
def get_active_users(debug_output_file_handle):
    ...
    user_records = []
    get_sql = "SELECT name, mail, theme, signature, signature_format, created, access, login, status, timezone, language, picture, init, data "
    get_sql += "FROM users "
...
def main():
    ...
    for active_user in active_users:
        ...
        #curr_module_project = drupal_6_json_get_key(curr_module_data, "project")
        #curr_module_version = drupal_6_json_get_key(curr_module_data, "version")
        ...
        output_file_handle.write(' ' + "<changed></changed>" + ENDL)
        output_file_handle.write(' ' + "</user>" + ENDL)

"""This script uses the following Environment Variables to setup a database connection to the
   drupal 7 website we are attempting to export. When setting the envrionment variables, there
...
current_website_url = os.environ.get("D7ET_CURR_SITE_URL")
...
def get_filepath(debug_output_file_handle, file_id):
    ...
    get_sql = "SELECT REPLACE(uri, 'public://', '') "
    get_sql += "FROM file_managed "
...
def get_content_types(debug_output_file_handle, content_types_to_exclude):
    """Query the database of the drupal 7 site to get all of the existing content types."""
...
def get_content_type_taxonomy_fields(debug_output_file_handle, content_type):
    ...
    debug_output_file_handle.write("get_content_type_taxonomy_fields sql statement: " + str(get_sql) + ENDL)
    debug_output_file_handle.flush()
    cursor.execute(get_sql)
    fieldrecords = cursor.fetchall()
    cursor.close()
...
def get_content_type_data(debug_output_file_handle, curr_content_type):
    ...
    for field in fields:
        ...
        curr_field_cardinality = field[4]
        if curr_field_cardinality > 1:
            continue

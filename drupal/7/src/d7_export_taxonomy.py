...
def get_vocabularies(debug_output_file_handle):
    ...
    get_sql = "SELECT vid, name FROM taxonomy_vocabulary ORDER BY vid"
...
def get_taxonomy_top_level(debug_output_file_handle, vocabulary_id):
    ...
    get_sql = "SELECT taxonomy_term_data.tid, name "
    get_sql = get_sql + "FROM taxonomy_term_data "
    get_sql = get_sql + "LEFT JOIN taxonomy_term_hierarchy "
    get_sql = get_sql + "ON (taxonomy_term_data.tid = taxonomy_term_hierarchy.tid) "
    get_sql = get_sql + "WHERE vid = " + str(vocabulary_id) + " "
    get_sql = get_sql + "AND taxonomy_term_hierarchy.parent = 0 "
    get_sql = get_sql + "ORDER BY name, weight, taxonomy_term_data.tid"
...
def printChildren(debug_output_file_handle, output_file_handle, vocabulary_id, vocabulary_name, depth, parent_id, parent_name):
    ...
    get_sql = get_sql + "ON (taxonomy_term_data.tid = taxonomy_term_hierarchy.tid) "
    get_sql = get_sql + "WHERE vid = " + str(vocabulary_id) + " "
    get_sql = get_sql + "AND taxonomy_term_hierarchy.parent = " + str(parent_id) + " "
    get_sql = get_sql + "ORDER BY name, weight, taxonomy_term_data.tid"
